...
const SideBar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const deleteChat = async (e, chatId) => {
    try {
      e.stopPropagation();
      const confirm = window.confirm("are you want to sure delete this chat");
      if (!confirm) return;
...
border-r border-[#80609F] backdrop-blur-3xl 
...
text-white bg-linear-to-r from-[#A456F7] to-[#3D81F6]
text-sm rounded-md cursor-pointer"
      >
        <span className="mr-2 text-xl">+</span> New Chat
      </button>

      {/*SEARCH CONVERSESIONS*/}
        ...
        {chats
          ...
          .map((chat) => (
            <div
              ...
              key={chat._id}
              className="p-2 px-4 dark:bg-[#57317C] border
